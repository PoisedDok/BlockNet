 ...
 * webview URI), a strict CSP meta tag, and a nonce on the built `<script>` tag. Every other
 * byte of the built HTML passes through untouched — this only prepares it for extension-host
 * serving, it doesn't restructure the build vite already produced. */
export function buildReadyHtml({ rawHtml, baseHref, cspSource, nonce }: ReadyHtmlParams): string {
  const csp = [`default-src 'none'`, `style-src ${cspSource}`, `font-src ${cspSource}`, `script-src 'nonce-${nonce}'`].join('; ');

  const withBase = rawHtml.replace('<head>', `<head>\n    <base href="${baseHref}">\n    <meta http-equiv="Content-Security-Policy" content="${csp}">`);
